...
var modulo = function(x, y) {
	...
	if(x >= y && x > 0) return modulo(x - y, y);
	if(x <= -y && x < 0) return modulo(x + y, y);
	return x;
};
...
var compareStr = function(str1, str2) {
	...
	if(str1 === '' || str2 === '' || str1[0] !== str2[0]) return false;
...
var letterTally = function(str, obj) {
	...
	let tally = letterTally(str.slice(1));
	if(str[0] in tally) {
		tally[str[0]] += 1;
	} else {
		tally[str[0]] = 1;
...
var augmentElements = function(array, aug) {
	...
	return [currElement].concat(augmentElements(array.slice(1), aug));
};
...
var minimizeZeroes = function(list) {
	if(list.length === 1) return list;
	if (list[0] === list[1] && list[0] === 0) return minimizeZeroes(list.slice(1));
...
var mergeSort = function(array) {
	...
	let sorted = [];
	while(idx1<mid && idx2 < array.length-mid){
	...
	if(idx1<mid){
		sorted = sorted.concat(arr1.slice(idx1));
	} else {
		sorted = sorted.concat(arr2.slice(idx2));
	}
	return sorted;
};
